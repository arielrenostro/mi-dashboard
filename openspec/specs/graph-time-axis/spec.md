...
`DashboardScreen` SHALL update the X range of each plot's `base_view` in `update_graph` using the buffer timestamps. `base_view` SHALL be stored per signal in `self.base_views` during `_create_graphs`.
